...
class Utils:
    ...
    def extract_time(self, sentence):
        ...
        if hours:
            if len(hours) == 2:
                return [
                    self.extract_time_from_word(
                        hours[0]), self.extract_time_from_word(
                        hours[1])]
            else:
...
def test():
    ...
    assert util.extract_time(example_hour1) == [
        '8:00', '9:00'], "Time Extraction Failed"
    assert util.extract_time(
        example_hour2) is None, "Failed to see if shop is closed"

    assert util.extract_shop_name(
        example_address1) == "William Small Centre", "Couldn't extract shop name"
    assert util.extract_shop_name(
        example_address2) == "York Lanes", "Couldn't extract shop name"
    print("Test Passed")
...
def debug():
    ...
    print(util.extract_time(example_hour2) is None)
    print("-" * 10)
